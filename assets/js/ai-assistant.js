...
function toggleAIChat() {
    ...
    if (!isVisible) {
        ...
        if (document.getElementById('ai-chat-body').innerHTML === "") {
            addAIMessage("assistant", "Bonjou! Mwen se **Piyay AI**. Mwen ka ede w jwenn nenpòt pwodwi nan boutik la. Kisa w ap chèche jodi a? 😊");
        }
    }
}
...
function addAIMessage(role, text) {
    ...
    // Netwaye tèks la (Bold)
    let formattedText = text.replace(/\*\*(.*?)\*\*/g, '<b>$1</b>');

    // 🔥 DETEKTE AK DESINE KAD PWODWI (CARD)
    // Fòma: [PRODUCT: Tit | Pri | Lyen | Imaj]
    let htmlContent = formattedText.replace(/\[PRODUCT:(.*?)\]/g, (match, content) => {
        ...
        return `
            <div style="background:white; border:1px solid #e2e8f0; border-radius:15px; overflow:hidden; margin-top:10px; width:100%; max-width:240px; box-shadow:0 10px 25px rgba(0,0,0,0.08); animation: fadeIn 0.3s ease;">
...
async function sendAIMessage() {
    ...
    try {
        const response = await fetch('/.netlify/functions/ai-chat', {
            ...
            body: JSON.stringify({ message: msg, catalog: aiCatalog })
        });
        ...
        if (data.choices && data.choices[0]) {
            ...
            addAIMessage('assistant', "Eskize m, mwen pa ka reponn kounye a. Eseye ankò.");
        }
    } catch (err) {
        if (document.getElementById('ai-typing')) document.getElementById('ai-typing').remove();
        addAIMessage('assistant', "Mwen gen yon ti pwoblèm koneksyon.");
    }
}
