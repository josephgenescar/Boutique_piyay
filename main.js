...
async function submitOrder() {
    ...
    if (!name || !phone || !zone) return alert("Tanpri ranpli Non, WhatsApp, ak Zòn Livrezon.");
    ...
    summary.innerHTML = `
        <div id="receipt-to-download" style="background:white; padding:20px; border:1px solid #eee;">
            <div style="text-align:center; border-bottom:2px solid #0f172a; padding-bottom:10px; margin-bottom:15px;">
                ...
                <h2 style="margin:5px 0 0 0; font-size:18px; text-transform:uppercase;">RESI ACHTE / INVOICE</h2>
            ...
            <div style="text-align:center; font-size:9px; color:#aaa; margin-top:20px; border-top:1px solid #eee; padding-top:10px;">Sponsorisé par: Rivayot-Tech Entreprise</div>
        </div>
        ...
        <button onclick="location.reload()" style="width:100%; background:none; border:none; color:#64748b; cursor:pointer; font-size:12px; text-decoration:underline;">Nouvo Kòmand</button>
    `;
}
...
async function downloadReceipt(){
    ...
    link.href = canvas.toDataURL('image/png'); link.download = `Resi-Boutique-Piyay-${Date.now()}.png`; link.click();
}
...
async function liveSearch() {
    ...
    try {
        ...
        else drop.innerHTML = results.map(r => `<a href="${r.url}" style="display:flex; padding:10px; text-decoration:none; color:#333; border-bottom:1px solid #eee;"><img src="${r.image}" style="width:40px; height:40px; object-fit:cover; margin-right:10px;"><div style="flex:1;"><strong>${r.title}</strong><br><span style="color:#ff4747; font-size:12px;">${r.price} HTG</span></div></a>`).join('');
